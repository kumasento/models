...
_DATA_URL = 'http://vision.stanford.edu/Datasets/Stanford40.zip'
...
def _get_filenames_and_classes(dataset_dir):
  ...
  """
  images_root = os.path.join(dataset_dir, 'JPEGImages')
  photo_filenames = []
  class_names = set()
  for filename in os.listdir(images_root):
    ...
    class_name = _get_class_name(filename)
      ...
      for shard_id in range(_NUM_SHARDS):
        ...
        with tf.python_io.TFRecordWriter(output_filename) as tfrecord_writer:
          ...
          for i in range(start_ndx, end_ndx):
            ...
            class_name = _get_class_name(filenames[i].split('/')[-1])
...
def _is_uncompressed(dataset_dir):
  if not tf.gfile.Exists(os.path.join(dataset_dir, 'JPEGImages')):
    return False
...
def run(dataset_dir):
  ...
  train_images_path = os.path.join(dataset_dir, 'ImageSplits', 'train.txt')
  test_images_path = os.path.join(dataset_dir, 'ImageSplits', 'test.txt')
  ...
  training_filenames = [name for name in photo_filenames
                        if _filename(name) in train_images]
  validation_filenames = [name for name in photo_filenames
                          if _filename(name) in test_images]
  ...
  print('\nFinished converting the Stanford 40 Actions dataset!')
